...
namespace Arvin
{
    ...
    {
        // 图片
        [FormerlySerializedAs("CloseMipMap")]
        [Tooltip("默认关闭图片的Mip Map")]
        public bool Texture_CloseMipMap = true;

        [FormerlySerializedAs("DefaultFormat")]
        [Tooltip("默认的图片压缩格式")]
        ...
        // 音效
        [FormerlySerializedAs("DefaultAudioClipFormat")]
        [Tooltip("默认的短音效处理")]
        ...
        [FormerlySerializedAs("DefaultMusicFormat")]
        [Tooltip("默认中长音频短处理")]
        ...
        [FormerlySerializedAs("OptimizeGameObjects")]
        [Tooltip("优化骨骼节点")]
        public bool Model_OptimizeGameObjects = true;

        [FormerlySerializedAs("ImportMaterials")]
        [Tooltip("不需要导入Material")]
        public bool Model_ImportMaterials = false;

        [FormerlySerializedAs("ImportCameras")]
        [Tooltip("是否导入摄像机")]
        public bool Model_ImportCameras = false;

        [FormerlySerializedAs("ImportLights")]
        [Tooltip("是否导入灯光")]
        ...
        [FormerlySerializedAs("GenerateSecondaryUV")]
        [Tooltip("是否导入 UV2")]
